  ...
  try {
    ...
    if (getFile) {
      if (getFile?.selectedFile !== req.body.selectedFile) {
        ...
        const new_updatedPost = await PostMemories.findByIdAndUpdate(
          req.params.id,
          { ...req.body, _id: req.body.id, selectedFile: new_public_id },
          { new: true }
        );
    ...
    const updatedPost = await PostMemories.findByIdAndUpdate(
      req.params.id,
